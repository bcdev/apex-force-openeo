...
**The CDSE Sentinel-2 L1C STAC collection that we use as input for FORCE level 2 processing is hosted at [https://stac.dataspace.copernicus.eu/v1/collections/sentinel-2-l1c](https://stac.dataspace.copernicus.eu/v1/collections/sentinel-2-l1c)** .
In the following, we show how this collection may be queried to obtain inputs for FORCE level 2 processing.
...
# ATTENTION: Whether the second date in the search is inclusive or exclusive depends on the query method (see below).
...
It is strongly recommended to use an external query method (such as `pystac_client`), because using the `query_stac`
process in combination with `force_level2` is limited to small numbers of items.
