...
The integration of the [FORCE](https://force-eo.readthedocs.io/en/latest/) processing engine in CDSE provides access to [a subset of FORCE's processing functionality](features.qmd) via the [openEO API on CDSE](https://eoap.github.io/).
...
<!-- TODO: update syntax when custom processes have been tested -->
::: {.callout-warning}
The force custom process syntax is not fully determined.
:::
...
<!-- TODO: Link examples section when done -->
For a full example, see \[TODO\].
...
For an overview of the FORCE functionality made available via openEO, see [features](features.qmd).
...
The EOAP is taylored and tested with the CDSE openEO backend. However, it may be possible to run the EOAP in a different environment.
See [EOAP](eoap.qmd) for more information.
