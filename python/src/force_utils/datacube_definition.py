...
class ForceDataCubeDefinition:
    ...
    def __init__(self, data_cube_root: Path | str):
        ...
        with open(self._data_cube_root / self.DATACUBE_DEFINITION_FILENAME) as fp:
            ...
            self.grid_origin_lon = self._parse_data_cube_definition_entry(
                fp.readline(), float
            )
            self.grid_origin_lat = self._parse_data_cube_definition_entry(
                fp.readline(), float
            )
            self.grid_origin_x = self._parse_data_cube_definition_entry(
                fp.readline(), float
            )
            self.grid_origin_y = self._parse_data_cube_definition_entry(
                fp.readline(), float
            )
            self.tile_size = int(
    ...
    def _generate_stac_assets(self, tile: str) -> Dict[str, pystac.Asset]:
        # TODO add data cube prj file as an asset
        ...
        for asset_path in tile_path.glob("*"):
            ...
            with rasterio.open(asset_path) as ds:
                ...
                ), f"Multiple scales found for asset {asset_path.resolve()}"
                asset.ext.raster.scale = scale
                offset = ds.offsets[0]
                assert all(
                    [o == offset for o in ds.offsets]
                ), f"Multiple offsets found for asset {asset_path.resolve()}"
